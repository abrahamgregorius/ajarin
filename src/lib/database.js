...
export async function getUserRanking(userId) {
    ...
    return {
        data: { rank: (count || 0) + 1, study_hours: userProfile.study_hours },
        error,
    };
}
...
export async function getTodaysHighlights() {
    ...
    try {
        ...
        const totalStudyHours =
            studyHoursData?.reduce(
                (sum, profile) => sum + (profile.study_hours || 0),
                0
            ) || 0;
        ...
        if (studentsError || hoursError || purchasesError || totalError) {
            console.error("Error fetching today's highlights:", {
                studentsError,
                hoursError,
                purchasesError,
                totalError,
            ...
            return {
                ...
                error:
                    studentsError || hoursError || purchasesError || totalError,
            };
        }
...
export async function rateVideo(videoId, userId, rating) {
    const { data, error } = await supabase.from("video_ratings").upsert(
        {
            video_id: videoId,
            user_id: userId,
            rating: rating,
...
export async function getUserStatistics(userId) {
    try {
        ...
        if (profileError) {
            ...
            return {
                data: { totalHours: 0, completedVideos: 0 },
                error: profileError,
            };
        }
        ...
        return {
            data: {
                totalHours:
                    Math.round((profile?.study_hours || 0) * 60 * 10) / 10, // Convert hours to minutes and round to 1 decimal
