...
// Submit a new video (creator/admin only)
export async function submitVideo(videoData) {
  const { data, error } = await supabase
    ...
    .insert({
      ...
      video_url: videoData.youtube_url, // For legacy compatibility
...
// Get videos submitted by a specific creator
...
// Get all pending videos (admin only)
...
export async function getAllVideos(approvalFilter = null) {
  let query = supabase
    .from("videos")
    .select(`
      *,
  ...
  if (approvalFilter !== null) {
    query = query.eq("approved", approvalFilter);
  }
...
// Reject/delete a video (admin only)
...
// Delete a video (creator can delete own pending videos)
